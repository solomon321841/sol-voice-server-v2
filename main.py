...
# - NATURAL_SPEECH: wait for full assistant text, optionally punctuate with LLM, then call TTS once
...
NATURAL_SPEECH = os.getenv("NATURAL_SPEECH", "1") == "1"
...
async def punctuate_with_llm(text: str) -> str:
    ...
    try:
        resp = await openai_client.chat.completions.create(
            model=GPT_MODEL,
            messages=[
                {"role": "system", "content": "Punctuate this text for spoken TTS output. Do not add new facts."},
